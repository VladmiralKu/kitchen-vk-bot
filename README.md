...
В режиме редактирования обычный новый текст заменяет заказ целиком, а текст с `+` добавляет позиции без удаления старых. Например: `+ К2 медовик - 1`.
...
Позиции до первой пустой строки попадают в `К1`. После пустой строки начинается следующий курс, то есть `десерт` из примера попадёт в `К2`. Можно также явно написать `К2` или `курс 2`.
