...
- Команды `/orders`, `/done`, `/users`, `/export`, `/gsheet`, `/stats`, `/stops`.
- Стоп-лист с хранением 12 часов.
- Excel-экспорт `.xlsx` с листами `Orders`, `Items`, `Events`, `Users`, `Stats`.
- Google Sheets-выгрузка заказов: один лист на месяц, одна строка на позицию заказа.
...
`/done` — выполненные заказы за сегодня.

`/done 2026-07-18` или `/done 18.07.2026` — выполненные заказы за выбранный день.

`/cancel <номер заказа>` — отменить заказ и убрать из активных, только admin.

`/edit <номер заказа>` — редактировать активный заказ, официант может редактировать свои заказы, admin любые.
В режиме редактирования обычный новый текст заменяет заказ целиком, а текст с `+` добавляет позиции без удаления старых. Например: `+ К2 1 медовик`.

`/export today` — Excel за сегодня, только admin.

`/export 2026-06-01 2026-06-24` — Excel за период, только admin.

`/gsheet` — выгрузить в Google Таблицу за вчера, только admin.

`/gsheet 2026-08-16` или `/gsheet 16.08.2026` — выгрузить выбранный день в Google Таблицу, только admin.
...
Позиции до первой пустой строки попадают в `К1`. После пустой строки начинается следующий курс, то есть `десерт` из примера попадёт в `К2`. Если количество стоит перед названием, например `2 капучино`, кухня получит две отдельные кнопки `капучино`. Можно также явно написать `К2` или `курс 2`.
...
`GOOGLE_SHEETS_SPREADSHEET_ID` — ID Google Таблицы из ссылки.

`GOOGLE_SERVICE_ACCOUNT_FILE` — путь к JSON-ключу сервисного аккаунта внутри контейнера, обычно `/app/secrets/google-service-account.json`.

`GOOGLE_SERVICE_ACCOUNT_JSON` — альтернатива файлу: JSON-ключ сервисного аккаунта одной переменной.
...
- Для Google Sheets нужно создать сервисный аккаунт Google, дать его email доступ к таблице как редактору и положить JSON-ключ на сервер в `secrets/google-service-account.json`.
